...
function draw() {
  ...
  let trS = rotVec();
  gl.uniform2fv(shProgram.iUserPoint, [1.0, 1.0]); //giving coordinates of user point
  gl.uniform3fv(shProgram.iUP, [trS[0] * 5, trS[1] * 5, trS[2] * 5]);
  gl.uniformMatrix4fv(shProgram.iModelViewMatrix, false, matAccum1);
  gl.uniformMatrix4fv(shProgram.iProjectionMatrix, false, projection);
  sphere.DrawSphere();
  if(panner){
    panner.setPosition(trS[0], trS[1], trS[2]);
  }
}
...
function initGL() {
  ...
  sphere.TextureBufferData(CreateSphereSurface())
...
function rotVec() {
  const alphaRad = rotVec3[0];
  const betaRad = rotVec3[1];
  const gammaRad = rotVec3[2];
